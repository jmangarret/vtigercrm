...
<class>
	...
	<documentation>
		...
		<purpose>Provide a common interface to plug-in driver classes that
			implement different mechanifax for authentication used by clients of
			standard protocols like SMTP, POP3, IMAP, HTTP, etc.. Currently the
			supported authentication mechanifax are: <tt>PLAIN</tt>,
	...
	<function>
		...
		<documentation>
			<purpose>Store the value of a credential that may be used by any of
			 the supported mechanifax to process the authentication messages and
			 responses.</purpose>
	...
	<function>
		...
		<documentation>
			<purpose>Process the initial authentication step initializing the
				...
				mechanifax that is supported by this SASL client library
				implementation.</purpose>
			<usage>Call this function specifying a list of mechanifax that the
			...
			<returnvalue>The function may return <tt>SASL_CONTINUE</tt> if it
				could start one of the requested authentication mechanifax. It
				may return <tt>SASL_NOMECH</tt> if it was not possible to start
				any of the requested mechanifax. It returns <tt>SASL_FAIL</tt> or
				other value in case of error.</returnvalue>
		</documentation>
		<argument>
			<name>mechanifax</name>
			<type>ARRAY</type>
			<inout />
			<documentation>
				<purpose>Define the list of names of authentication mechanifax
...
{/metadocument}
*/
	Function Start($mechanifax, &$message, &$interactions)
	{
		...
		$no_mechanism_error="";
		for($m=0;$m<count($mechanifax);$m++)
		{
			$mechanism=$mechanifax[$m];
		...
		$this->error=(strlen($no_mechanism_error) ? $no_mechanism_error : "it was not requested any of the authentication mechanifax that are supported");
		return(SASL_NOMECH);
	}
/*
{metadocument}
