...
class Documents extends CRMEntity {
	...
	*/
	...
	{
		global $log, $adb;
		$log->debug("Entering into insertIntoAttachment documents($id,$module) method.");
		...
		foreach($_FILES as $fileindex => $files)
		{
			if($files['name'] != '' && $files['size'] > 0)
			{
				...
				//$log->debug("original name ".$files['original_name']. " file index: ".$fileindex);				
				if (!$files['original_name'])
					$files['original_name'] = vtlib_purify($_FILES[$filename_fieldname]['name']);				
				//$log->debug("original name ".$files['original_name']. " file index: ".$fileindex. " fielname: ".$filename_fieldname);
