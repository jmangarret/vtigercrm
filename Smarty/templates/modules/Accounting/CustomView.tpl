...
{ldelim}
        ...
        var bAccountingIdExists = false;
		for(i=1;i<=9;i++) {ldelim}
			var columnvalue = document.getElementById("column"+i).value;
			if (columnvalue != null) {ldelim}
				if(columnvalue=="vtiger_accounting:accounting_id:accounting_id:Accounting_PaymentId:V") {ldelim}
					bAccountingIdExists = true;
				{rdelim}
			{rdelim}
		{rdelim}

		if(bAccountingIdExists == false) {ldelim}
			alert("{$MUSTHAVE_ACCOUNTING_ID_FIELD}");
			return false;
		{rdelim}
...
{* -> AXIALBLUE: Advanced Filter 5.3 compatibility *}

{if $AX_ADV_FILTER_PATCH eq "true"}
...
{* -> AXIALBLUE: Advanced Filter 5.3 compatibility *}
{if $AX_ADV_FILTER_PATCH neq "true"}
