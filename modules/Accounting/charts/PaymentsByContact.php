...
$sql = "SELECT CONCAT(vtiger_contactdetails.firstname, ' ', vtiger_contactdetails.lastname) as name, vtiger_accounting_payments.paid, vtiger_accounting.accountingcurrency, vtiger_accounting_payments.amount,vtiger_accounting_payments.paymentduedate,vtiger_accounting_payments.paymentdate ".
			"FROM vtiger_accounting ".
				...
				"INNER JOIN vtiger_contactdetails ".
					"ON vtiger_contactdetails.contactid = vtiger_contactdetails.accountingrelated1 ".
			"WHERE ".
				...
				"ORDER BY name, vtiger_accounting_payments.paid,vtiger_accounting.accountingcurrency";
...
while ( $row = $adb->fetchByAssoc($result)){
	...
	if($row['paid'] == '1') {
		$payments['payments'][$row['name']]['paid'] += $row['amount'];
	} else {
		$diffdays = diff_date_today($row['paymentduedate']);
		if($diffdays > 0) {
			$payments['payments'][$row['name']]['unpaid'] += $row['amount'];
		} else {
			$payments['payments'][$row['name']]['pending'] += $row['amount'];
		}
	}
