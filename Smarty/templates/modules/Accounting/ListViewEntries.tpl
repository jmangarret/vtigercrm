     ...
     <input name="current_page_boxes" id="current_page_boxes" type="hidden" value="{$CURRENT_PAGE_BOXES}">
                ...
                <td nowrap>
                	<table>
                		<tr>
                			<td>
                				<input onclick="changePartialView();" type="checkbox" name="partialpayments" id="partialpayments" value="" {$PARTIAL_PAYMENT_VIEW}>{$MOD.LBL_PARTIAL_PAYMENTS_VIEW}</input>
                			</td>
                		</tr>
                	</table>
                </td>
						...
						<td style="padding-left:5px;padding-right:5px">
							...
							<td>
								...
								{if $CV_EDIT_PERMIT neq 'yes'}
									<span class="small" disabled>{$APP.LNK_CV_EDIT}</span>
								{else}
									<a href="index.php?module={$MODULE}&action=CustomViewIndex&record={$VIEWID}&parenttab={$CATEGORY}">{$APP.LNK_CV_EDIT}</a>
								{/if}
								<span class="small">|</span>
			...
			{assign var="entity_id2" value=$entity_id|cleanEntityList}
			<tr bgcolor=white onMouseOver="this.className='lvtColDataHover'" onMouseOut="this.className='lvtColData'" id="row_{$entity_id2}">
			<td width="2%"><input type="checkbox" NAME="selected_id" id="{$entity_id2}" value= '{$entity_id2}' onClick="check_object(this)"></td>
			{foreach item=data from=$entity name="acc_headers"}
			{* vtlib customization: Trigger events on listview cell *}
			{assign var="omit_head" value="false"}
			{foreach item=H_ACC from=$OMIT_HEADERS}
				{if $H_ACC eq $smarty.foreach.acc_headers.index}
					{assign var="omit_head" value="true"}
				{/if}
			{/foreach}
			{if $omit_head eq "false"}
			...
			{*
			{if $PARTIAL_PAYMENT_VIEW eq "checked" && $OMIT_STATUS neq "" && $smarty.foreach.acc_headers.index neq $OMIT_STATUS}
				<td onmouseover="vtlib_listview.trigger('cell.onmouseover', $(this))" onmouseout="vtlib_listview.trigger('cell.onmouseout', $(this))">{$data}</td>
			{elseif $PARTIAL_PAYMENT_VIEW neq "checked"}
				<td onmouseover="vtlib_listview.trigger('cell.onmouseover', $(this))" onmouseout="vtlib_listview.trigger('cell.onmouseout', $(this))">{$data}</td>
			{/if}
			*}
			{* END *}
				...
				<td nowrap width="30%" align="center">
				 ...
				 <td align="right" width=40%>
				   <table border=0 cellspacing=0 cellpadding=0 class="small">
					...
					{*{if $SHOW_BALANCE_LINK eq 'true'}*}
					{*{if $ISADMIN eq 'true'}*}
				   		<a href="index.php?module=Accounting&return_module=Accounting&action=AccountingAjax&file=AccountingCharts&src_module=Accounting" target="_blank"><strong>{$MOD.LBL_CHART_BALANCE}</strong></a>
				   	{* {/if} *}
				   	{*{/if}*}
				 </td>
			      </tr>
