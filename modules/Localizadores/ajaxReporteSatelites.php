...
if ($_REQUEST["satelite"]!=""){
	$query="SELECT loc.localizadoresid,loc.localizador, con.contactid, con.firstname, con.lastname, loc.paymentmethod, loc.registrodeventasid, loc.procesado, loc.gds, loc.totalloc, bol.fecha_emision, bol.boleto1, bol.status
	...
	$else = 1;
	$query="SELECT loc.localizadoresid,loc.localizador, loc.contactoid, loc.paymentmethod, loc.registrodeventasid, loc.procesado, loc.gds, loc.totalloc, bol.fecha_emision, bol.boleto1, bol.status
...
?>
	<tr class="listViewEntries" data-id='<?=$row["localizadoresid"]?>' data-recordUrl='index.php?module=Localizadores&view=Detail&record=<?=$row["localizadoresid"]?>' id="Localizadores_listView_row_1">
		...
		<td class="listViewEntryValue wide" data-field-type="string" data-field-name="fecha_emision" nowrap><?=$row["fecha_emision"]?></td>
