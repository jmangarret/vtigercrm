							...
							{if $header eq $MOD.LBL_ADDITIONAL_INFO}
								   ...
								   <table id="payments_table" width="70%">
								   		<tr>
											...
											<th style="display: {$displayInv}">{$MOD.RelatedTo}</th>
											<th>{$MOD.LBL_PARTIAL_PAID_AMOUNT}</th>
									   		...
									   		{foreach from=$PAYMENTS item=PAYMENT name=foo}
									   			...
									   			{/if}
											   		<tr bgcolor="{$bgColor}">
											   			<td name="nPayment"><font size="+1">{$smarty.foreach.foo.index+1}</font></td>
											   			<td align="center">{$PAYMENT.ref}</td>
											   			<td align="center">{$PAYMENT.method}</td>
											   			<td align="center">{$PAYMENT.amount}</td>
											   			<td style="display: {$displayVAT}" align="center">{$PAYMENT.tax}</td>
...
{if $header neq 'Comments'}
				...
				{else}
					{if $keyfldname eq "uniqueid"}
						{assign var="DISPLAY_FLD2" value="none"}
					{else}
						{assign var="DISPLAY_FLD2" value=""}
					{/if}

					<td style="display: {$DISPLAY_FLD2}" class="dvtCellLabel" align=right width=25%>
						<input type="hidden" id="hdtxt_IsAdmin" value={$keyadmin}></input>{$label}
					</td>
				{/if}
				{if $EDIT_PERMISSION eq 'yes' && $display_type neq '2'}
					{* Performance Optimization Control *}
					{if !empty($DETAILVIEW_AJAX_EDIT) }
						{include file="modules/Accounting/DetailViewUI.tpl"}
					{else}
						{include file="modules/Accounting/DetailViewFields.tpl"}
					{/if}
					{* END *}
				{else}
					{include file="modules/Accounting/DetailViewFields.tpl"}
